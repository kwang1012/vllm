...
def main():
    metric = sys.argv[1]
    model = sys.argv[2]
    print(f"Plotting {metric} for {model} model")

    throughput_dict = {}
    itl_dict = {}
    bs_list = [8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096]
    ...
    for key in sorted(throughput_dict):
        if not throughput_dict[key]:
            continue
        if metric == "itl":
            ax.plot(bs_list, itl_dict[key], label=key)
        else:
            ax.plot(bs_list, throughput_dict[key], label=key)
    if metric == "itl":
    ...
    ax.legend()
    plt.grid()
    fig.savefig(f"{metric}_{model}.png")
    print(f"Saved {metric}_{model}.png")
